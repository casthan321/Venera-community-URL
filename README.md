...
“新源工坊”生成漫画源后，可一键把源文件和对应的 `index.json` 条目发布到本仓库。发布不是在浏览器里保存 GitHub 令牌：网页把生成结果交给受控的服务端，服务端重新检查元数据、搜索、详情、章节图片、已声明的可选能力和敏感信息，再用一次原子提交同时更新两个文件。提交成功后，GitHub Actions 会继续用官方 `venera_cli` 做最终验证。
...
腾讯动漫与 NoyManga 1.1.0 继续按完整能力清单严格检查，不能静默丢失登录、收藏或发现；NoyManga 还必须保留手动签到和默认可控的自动签到设置。其他一键发布的新源即使只有基础阅读能力，也可以通过基础检查，但不会因此凭空获得登录、收藏、发现或签到功能。
